...
int main() {
  ...
  heapSort(tBST,1000);
  writeFile(false,"Heap Sort Best Case: ");
  heapSort(tAVG,1000);
  writeFile(false,"Heap Sort Average Case: ");
  heapSort(tWST,1000);
  writeFile(false,"Heap Sort Worst Case: ");

  //printArrays(tBST,tAVG,tWST);

  if(!verifySort(tBST) || !verifySort(tAVG) || !verifySort(tWST)) cout << "Error with Heap Sort" << endl;
